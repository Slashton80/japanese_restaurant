...
public class CisUtility {
 ...
 appended to it.
    ...
    /**
     * Method to get a LocalDateTime value from the user.
     *
     * @param prompt The user prompt to enter the date and time
     * @param format The expected format for the date and time (e.g., "yyyy-MM-dd HH:mm")
     * @return A LocalDateTime object representing the entered date and time
     * @author Sherri Ashton
     * @since 2023-11-30
     */
    public static LocalDateTime getInputDateTime(String prompt, String format) {
        ...
        while (!validInput) {
            try {
                String userInput = getInputString(prompt);
                dateTime = LocalDateTime.parse(userInput, formatter);
                validInput = true;
            } catch (DateTimeParseException e) {
                System.out.println("Invalid date and time format. Please enter in the format: " + format);
            }
        }
